...
if [ -x "$(command -v apk)" ]; then
    OS='alpine'
    PACKAGE_UPDATE='sudo apk update'
    PACKAGE_INSTALL='suod apk add --no-cache'
    ...
    OS='fedora'
    PACKAGE_INSTALL='sudo dnf install'
    DEPS="${DEPS_FEDORA[@]}"
    ...
    OS='suse'
    PACKAGE_INSTALL='sudo zypper install'
    DEPS="$DEPS_SUSE"
elif [ -x "$(command -v pacman)" ]; then
    OS='arch'
    PACKAGE_UPDATE='sudo pacman -Syy'
