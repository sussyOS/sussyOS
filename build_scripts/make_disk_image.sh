...
mkfs.fat -n "SUSSYOS" $TARGET_PARTITION >/dev/null
...
# copy files
echo "Copying files to ${TARGET_PARTITION} (mounted on /tmp/susos)..."
mkdir -p /tmp/susos
mount ${TARGET_PARTITION} /tmp/susos
cp ${BUILD_DIR}/kernel.bin /tmp/susos
cp test.txt /tmp/susos
mkdir /tmp/susos/test
cp test.txt /tmp/susos/test
umount /tmp/susos
