...
{
    g_PicMask = newMask;
    i8259_outb(PIC1_DATA_PORT, g_PicMask & 0xFF);
    i8259_iowait();
    i8259_outb(PIC2_DATA_PORT, g_PicMask >> 8);
    i8259_iowait();
}
...
void i8259_Configure(uint8_t offsetPic1, uint8_t offsetPic2, bool autoEoi)
{
    ...
    // initialization control word 1
    i8259_outb(PIC1_COMMAND_PORT, PIC_ICW1_ICW4 | PIC_ICW1_INITIALIZE);
    i8259_iowait();
    i8259_outb(PIC2_COMMAND_PORT, PIC_ICW1_ICW4 | PIC_ICW1_INITIALIZE);
    i8259_iowait();
    ...
    i8259_outb(PIC1_DATA_PORT, 0x4);             // tell PIC1 that it has a slave at IRQ2 (0000 0100)
    i8259_iowait();
    i8259_outb(PIC2_DATA_PORT, 0x2);             // tell PIC2 its cascade identity (0000 0010)
    i8259_iowait();
    ...
    i8259_outb(PIC1_DATA_PORT, icw4);
    i8259_iowait();
    i8259_outb(PIC2_DATA_PORT, icw4);
    i8259_iowait();
...
uint16_t i8259_ReadIrqRequestRegister()
{
    ...
    return ((uint16_t)i8259_inb(PIC2_COMMAND_PORT)) | (((uint16_t)i8259_inb(PIC2_COMMAND_PORT)) << 8);
}
